...
def main(config):
    ...
    to_na_all_lakes = [ 'mwp-count', 'mwp-insufficientData', 'mwp-noWater',
        'mwp-surfaceWater', 'mwp-Flood', 'gwp-Water',
    ...
    for lake_id in no_disruption_dict.keys():
        if lake_id not in final_cleaned_dict:
            ...
            length_removed_lakes.append({
                ...
                'Lake_area': df['Lake_area'].iloc[0],
                'total_points': len(df),
                'reason': 'Insufficient data points (< 50)',
                'actual_points': len(df)
            })

    print(f"\nZusätzlich entfernte Seen (< 50 Datenpunkte): {len(length_removed_lakes)}")
    if length_removed_lakes:
        length_removed_df = pd.DataFrame(length_removed_lakes)
        print(f"Entfernte Seen (< 50 Punkte):\n{length_removed_df[['Lake_name', 'Lake_area', 'actual_points']]}")
